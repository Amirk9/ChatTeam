...
export default function ShareDialog({ fileId, filename, onClose, onShared }) {
  ...
  return (
    <div className="fixed inset-0 z-30 flex items-center justify-center bg-black/40" onClick={onClose}>
      <form onSubmit={submit} onClick={(e) => e.stopPropagation()} className="w-96 max-w-[90vw] bg-white rounded-lg shadow-xl p-4">
        ...
        <select value={channelId} onChange={(e) => setChannelId(e.target.value)} className="w-full border border-gray-300 rounded-md px-2 py-1.5 text-sm mb-3">
          {channels.map((c) => (
        ...
        <input value={content} onChange={(e) => setContent(e.target.value)} placeholder="Say something about this file..." className="w-full border border-gray-300 rounded-md px-2 py-1.5 text-sm mb-3" />
        {error ? <p className="text-xs text-red-600 mb-2">{error}</p> : null}
        <div className="flex justify-end gap-2">
          <button type="button" onClick={onClose} className="text-xs px-3 py-1.5 rounded border border-gray-300">Cancel</button>
