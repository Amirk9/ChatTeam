    ...
    <div className="p-6 max-w-xl font-sans">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">{user.email}{user.emailVerified ? ' · verified' : ' · unverified'}</p>
        </div>
      </div>
      ...
      <form onSubmit={save} className="bg-white border border-gray-200 rounded-xl p-4 space-y-1">
        <label className="text-sm font-semibold">Display name</label>
        <Field value={displayName} onChange={(e) => setDisplayName(e.target.value)} />
        <label className="text-sm font-semibold">Status</label>
        <select value={status} onChange={(e) => setStatus(e.target.value)} className="block w-full my-2 px-3 py-2.5 border border-gray-300 rounded-lg text-sm">
