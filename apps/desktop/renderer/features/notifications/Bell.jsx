...
export default function Bell() {
  ...
  return (
    <div className="relative">
      ...
      {open ? (
        <div className="absolute right-0 top-9 w-80 max-h-96 overflow-y-auto bg-white text-[#1d1c1d] rounded-lg shadow-xl border border-gray-200 z-50">
          <div className="flex items-center px-4 py-2 border-b border-gray-100">
            ...
            <button onClick={() => { markAllRead(); }} className="text-xs text-[#1264A3] hover:underline">Mark all read</button>
          </div>
          {notifications.length === 0 ? (
            ...
            <ul className="divide-y divide-gray-100">
              {notifications.map((n) => (
                <li key={n.id} className={`px-4 py-2.5 text-sm ${n.is_read ? 'text-gray-500' : 'bg-blue-50/50'}`}>
