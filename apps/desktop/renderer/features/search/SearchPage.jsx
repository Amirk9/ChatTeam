    ...
    <div className="flex-1 overflow-y-auto p-6 max-w-4xl w-full mx-auto">
      <form onSubmit={(e) => { e.preventDefault(); setParams({ q: q.trim() }); run(q, tab); }} className="flex gap-2 mb-3">
        <input
          ...
          className="flex-1 border border-gray-300 rounded-md px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-[#611f69]"
        />
      ...
      <div className="flex gap-1 mb-4">
        {TABS.map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`text-xs px-3 py-1 rounded-full border capitalize ${tab === t ? 'bg-[#611f69] text-white border-[#611f69]' : 'border-gray-300 hover:bg-gray-100'}`}
          >
            {t}
          </button>
        ))}
      ...
      {!loading && !data && !error ? (
        <p className="text-sm text-gray-500">Slack-style search: <code>from:@user</code> · <code>in:#channel</code> · <code>"exact phrase"</code> · <code>has:file</code> · <code>before:</code>/<code>after:</code></p>
      ) : null}
    </div>
  );
}
      ...
      {show('messages') && (
        <section>
          ...
          {msgs.length === 0 ? <p className="text-sm text-gray-400">No messages found.</p> : msgs.map((m) => (
            <button key={m.id} onClick={() => onJump(m)} className="w-full text-left border border-gray-200 rounded-lg p-3 mb-2 hover:border-[#611f69]">
              <p className="text-xs text-gray-500 mb-1">
                <span className="font-semibold text-black">{m.sender.displayName}</span> in #{m.channelName || channelName(m.channelId) || 'channel'} · {new Date(m.createdAt).toLocaleString()}
                {m.hasFile ? ' · 📎' : ''}
              </p>
      ...
      {show('users') && (
        <section>
          ...
          {((tab === 'all' ? data.users?.items : data.items) || []).map((u) => (
            <div key={u.id} className="flex items-center gap-2 border border-gray-200 rounded-lg p-2 mb-1 text-sm">
      ...
      {show('channels') && (
        <section>
          ...
          {((tab === 'all' ? data.channels?.items : data.items) || []).map((c) => (
            <Link key={c.id} to="/" className="block border border-gray-200 rounded-lg p-2 mb-1 text-sm hover:border-[#611f69]">
      ...
      {show('files') && (
        <section>
          ...
          {((tab === 'all' ? data.files?.items : data.items) || []).map((f) => (
            <a key={f.id} href={`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/files/${f.id}?download=1`} className="block border border-gray-200 rounded-lg p-2 mb-1 text-sm hover:border-[#611f69]">
